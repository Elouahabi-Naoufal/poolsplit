...
import { acceptInvitationService } from "@/services/groups";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ invitationId: string }> }) {
  try {
    ...
    if (!result.ok) {
      const status = result.error === "inviteNotFound" || result.error === "groupMissing" ? 404 : result.error === "notYourInvite" ? 403 : 409;
